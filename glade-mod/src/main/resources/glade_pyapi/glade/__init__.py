...
def _callable_payload(function):
    if not isinstance(function, _types.FunctionType):
        raise TypeError("glade.spawn() requires a Python function")
    referenced = {}
    modules = {}
    for name in function.__code__.co_names:
        if name not in function.__globals__ or name in {"glade", "__builtins__"}:
            continue
...
def spawn(callable):
    ...
    return _SpawnHandle(_glade_concurrency.spawn(_callable_payload(callable)))
