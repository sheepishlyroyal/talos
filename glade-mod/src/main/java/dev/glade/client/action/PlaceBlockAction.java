...
public final class PlaceBlockAction extends SimpleTask {
    ...
    private void prepare(MinecraftClient client) {
        ...
        aim = new AimController(client, GladeClient.humanizer().rotation(), profile,
    ...
    private void execute(MinecraftClient client) {
        ...
        waitUntil = ticks + GladeClient.humanizer().timing().sampleDelayTicks(profile, 0.35, rng);
        state = State.OBSERVE;
    }
    ...
    private void retry(String message) {
        ...
        waitUntil = ticks + 1 + GladeClient.humanizer().timing().sampleDelayTicks(profile, 0.55, rng);
        state = State.BACKOFF;
    }
