...
public final class EntitySelector {
    ...
    private boolean applyArgument(String key, String value, String[] error) {
        switch (key) {
            case "type" -> {
                ...
                if (tag) {
                    TagKey<EntityType<?>> typeTagKey = TagKey.of(RegistryKeys.ENTITY_TYPE, id);
    ...
    public boolean matchesFilters(Entity entity) {
        if (typeId != null) {
            if (Registries.ENTITY_TYPE.get(typeId) != entity.getType()) return false;
        }
        if (typeTag != null && !entity.getType().isIn(typeTag)) return false;
        ...
        for (TagKey<EntityType<?>> excludedTypeTag : excludedTypeTags) {
            if (entity.getType().isIn(excludedTypeTag)) return false;
        }
        for (String tag : requiredTags) {
            if (tag.isEmpty() ? !entity.getCommandTags().isEmpty() : !entity.getCommandTags().contains(tag)) {
                return false;
            }
        }
    ...
    public boolean matches(Entity entity, Entity self) {
        boolean kindMatches = switch (kind) {
            ...
            case PLAYERS_ALL, PLAYER_NEAREST, PLAYER_RANDOM -> entity instanceof PlayerEntity;
            case ENTITIES, ENTITY_NEAREST -> true;
        };
        return kindMatches && matchesFilters(entity)
                && withinDistance(Math.sqrt(entity.squaredDistanceTo(self)));
    }

    /** Returns all loaded matches in selector order with the selector's limit applied. */
    public List<Entity> select(MinecraftClient client, boolean excludeSelf) {
        ...
        Comparator<Entity> byDistance = Comparator.comparingDouble(self::squaredDistanceTo);
        switch (ordering) {
