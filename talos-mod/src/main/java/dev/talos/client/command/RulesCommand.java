...
public final class RulesCommand {
    ...
    public static LiteralArgumentBuilder<FabricClientCommandSource> onNode() {
        ...
        for (Trigger trigger : Trigger.values()) {
            ...
            switch (trigger.kind) {
                ...
                case ENTITY_COUNT -> {
                    var radius = ClientCommandManager.argument("radius",
                    ...
                    node.then(ClientCommandManager.argument("selector", SelectorArgumentType.selector())
                            .then(ClientCommandManager.literal("radius").then(radius)));
                }
                ...
                case BLOCK_COUNT -> {
                    ...
                    node.then(ClientCommandManager.argument("block", IdArgumentType.blockId())
                            .then(ClientCommandManager.literal("radius").then(radius)));
                }
                case BLOCK_PRESENCE -> {
                    var radius = ClientCommandManager.argument("radius",
    ...
    private static LiteralArgumentBuilder<FabricClientCommandSource> atComparisons(Trigger trigger,
        ...
        var at = ClientCommandManager.literal("at");
        ...
        if (EventRuleEngine.acceptsSpatialRadius(trigger)) {
            z.then(ClientCommandManager.literal("radius").then(attachComparisons(
                    ClientCommandManager.argument("at_radius", DoubleArgumentType.doubleArg(0.0, 512.0)),
                    trigger, (context, rule) -> {
    ...
    private static net.minecraft.util.math.Vec3d resolvePoint(
        ...
        if (local) return RaycastMath.local(player.getEyePos(), player.getYaw(),
                player.getPitch(), x.value(), y.value(), z.value());
        var feet = player.getEntityPos();
