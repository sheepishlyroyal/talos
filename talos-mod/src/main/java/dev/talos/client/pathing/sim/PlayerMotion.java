 ...
 * state. Depending on {@link CollisionGetter} (not World) keeps it runnable in unit tests.
 */
public final class PlayerMotion {
    ...
    public static MotionState step(CollisionGetter world, MotionState state, Input input) {
    ...
    public static MotionState step(CollisionGetter world, MotionState state, Input input,
        ...
        AABB box = state.box(state.position());
        ...
        boolean climbing = fluid == MotionState.Fluid.NONE && climbable(world.getBlockState(
                BlockPos.containing(state.position().x, state.position().y + 1.0E-4,
                        state.position().z)));
        ...
        Vec3 velocity = state.velocity().add(inputVelocity(input, acceleration, inputFactor));
        ...
        if (env.flow().lengthSqr() > 0.0) {
            Vec3 push = env.flow().scale(
                    fluid == MotionState.Fluid.LAVA ? LAVA_FLOW_PUSH : WATER_FLOW_PUSH);
            if (Math.abs(velocity.x) < 0.003 && Math.abs(velocity.z) < 0.003
                    && push.length() < MIN_FLUID_PUSH && push.lengthSqr() > 0.0) {
                push = push.normalize().scale(MIN_FLUID_PUSH);
            }
        ...
        if (jumped) {
            ...
            if (input.sprint()) {
                ...
                velocity = velocity.add(-Mth.sin(radians) * SPRINT_JUMP_IMPULSE, 0.0,
                        Mth.cos(radians) * SPRINT_JUMP_IMPULSE);
            }
        ...
        if (climbing) {
            ...
            velocity = new Vec3(Mth.clamp(velocity.x, -0.15, 0.15), cy,
                    Mth.clamp(velocity.z, -0.15, 0.15));
        }
        ...
        Vec3 requested = input.sneak() && state.onGround()
        ...
        if (smothered) {
            requested = new Vec3(requested.x * env.slowFactor().x,
                    requested.y * env.slowFactor().y, requested.z * env.slowFactor().z);
        }
        Vec3 resolved = collide(world, box, requested);
        ...
        if (state.onGround() && profile.stepHeight() > 0.0
            ...
            Vec3 stepped = collide(world, box,
                    new Vec3(requested.x, profile.stepHeight(), requested.z));
            Vec3 settled = collide(world, box.move(stepped),
                    new Vec3(0.0, -profile.stepHeight(), 0.0));
            stepped = stepped.add(settled);
            if (stepped.horizontalDistanceSqr() > resolved.horizontalDistanceSqr() + EPSILON) {
                resolved = stepped;
            }
        }
        ...
        Vec3 newPosition = state.position().add(resolved);
        ...
        if (onGround && !input.sneak() && !smothered && velocity.y < 0.0
                && world.getBlockState(BlockPos.containing(newPosition.x,
                        newPosition.y - 0.5000001, newPosition.z)).is(Blocks.SLIME_BLOCK)) {
            vy = -velocity.y;
        }
        ...
        return new MotionState(newPosition, new Vec3(vx, vy, vz), onGround,
    ...
    private record Environment(MotionState.Fluid fluid, Vec3 slowFactor, Vec3 flow,
            int bubble) {}

    private static Environment scanEnvironment(CollisionGetter world, AABB box) {
                ...
                for (int z = minZ; z <= maxZ; z++) {
                    ...
                    if (!fluidState.isEmpty()) {
                        ...
                        if (surface >= box.minY) {
                            ...
                            flow = flow.add(fluidState.getFlow(world, pos));
                            flowCells++;
                        }
                    }
                    Vec3 factor = slowFactor(block);
                    if (factor != null) {
                        slow = slow == null ? factor
                                : new Vec3(Math.min(slow.x, factor.x),
                                        Math.min(slow.y, factor.y), Math.min(slow.z, factor.z));
                    }
                    if (block.is(Blocks.BUBBLE_COLUMN)) {
                        bubble = block.getValue(net.minecraft.world.level.block.BubbleColumnBlock.DRAG_DOWN) ? -1 : 1;
                    }
                }
            }
        }
        if (flowCells > 0) {
            flow = flow.scale(1.0 / flowCells);
            ...
            if (depth < 0.4) flow = flow.scale(depth);
        }
        return new Environment(fluid, slow, flow, bubble);
    }

    /** Vanilla Entity.slowMovement callers: cobweb, sweet berry bush, powder snow. */
    private static Vec3 slowFactor(BlockState state) {
        if (state.is(Blocks.COBWEB)) return new Vec3(0.25, 0.05, 0.25);
        if (state.is(Blocks.SWEET_BERRY_BUSH)) return new Vec3(0.8, 0.75, 0.8);
        if (state.is(Blocks.POWDER_SNOW)) return new Vec3(0.9, 1.5, 0.9);
        return null;
    }
    ...
    static boolean climbable(BlockState state) {
        return state.is(BlockTags.CLIMBABLE) || state.is(Blocks.LADDER)
                || state.is(Blocks.VINE) || state.is(Blocks.SCAFFOLDING);
    }

    static boolean isWater(FluidState state) {
        return state.is(FluidTags.WATER) || state.getType() == Fluids.WATER
                || state.getType() == Fluids.FLOWING_WATER;
    }

    static boolean isLava(FluidState state) {
        return state.is(FluidTags.LAVA) || state.getType() == Fluids.LAVA
     ...
     */
    private static Vec3 clipAtLedge(CollisionGetter world, AABB box, Vec3 movement, double stepHeight) {
        double probe = Math.max(stepHeight, 0.6);
        double x = movement.x;
        double z = movement.z;
        while (x != 0.0 && unsupported(world, box.move(x, -probe, 0.0))) x = shrinkStep(x);
        while (z != 0.0 && unsupported(world, box.move(0.0, -probe, z))) z = shrinkStep(z);
        while (x != 0.0 && z != 0.0 && unsupported(world, box.move(x, -probe, z))) {
            x = shrinkStep(x);
            z = shrinkStep(z);
        }
    ...
    private static boolean unsupported(CollisionGetter world, AABB box) {
    ...
    public static boolean hitboxFits(CollisionGetter world, MotionState.Pose pose, Vec3 pos) {
        AABB box = MotionState.box(pose, pos);
    ...
    private static Vec3 inputVelocity(Input input, double speed, double inputFactor) {
        double x = input.strafe() * inputFactor;
        double z = input.forward() * inputFactor;
        double lengthSquared = x * x + z * z;
        if (lengthSquared < EPSILON) return new Vec3(0.0, 0.0, 0.0);
        ...
        return new Vec3(x * cos - z * sin, 0.0, z * cos + x * sin);
    }

    private static Vec3 collide(CollisionGetter world, AABB box, Vec3 movement) {
        ...
        for (VoxelShape shape : world.getBlockCollisions(null, box.expandTowards(movement))) {
        ...
        for (Direction.Axis axis : Direction.axisStepOrder(movement)) {
            ...
            if (amount != 0.0) {
                amount = Shapes.collide(axis, box.move(result), collisions, amount);
                result = result.with(axis, amount);
            }
        }
        return result;
    }
     ...
     */
    private static double velocityMultiplier(CollisionGetter world, Vec3 position) {
        BlockPos feet = BlockPos.containing(position);
        float at = world.getBlockState(feet).getBlock().getSpeedFactor();
        if (at != 1.0F) return at;
        BlockPos below = BlockPos.containing(position.x, position.y - 0.5000001, position.z);
        return world.getBlockState(below).getBlock().getSpeedFactor();
    }

    /** Vanilla LivingEntity jump scaling: honey and friends stunt the jump to half height. */
    private static double jumpMultiplier(CollisionGetter world, Vec3 position) {
        BlockPos feet = BlockPos.containing(position);
        float at = world.getBlockState(feet).getBlock().getJumpFactor();
        if (at != 1.0F) return at;
        BlockPos below = BlockPos.containing(position.x, position.y - 0.5000001, position.z);
        return world.getBlockState(below).getBlock().getJumpFactor();
    }

    /** Slipperiness of the block under these feet (0.6 normal ground, 0.98 ice). */
    public static double slipperinessBelow(CollisionGetter world, Vec3 position) {
        BlockPos below = BlockPos.containing(position.x, position.y - 0.001, position.z);
