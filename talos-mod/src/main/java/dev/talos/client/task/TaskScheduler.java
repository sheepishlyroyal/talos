...
public final class TaskScheduler {
    ...
    private final LinkedHashMap<String, TalosTask> tasks = new LinkedHashMap<>();
    ...
    public void tick() {
        ...
        while (!runnable.isEmpty()) {
            ...
            Iterator<Map.Entry<String, TalosTask>> iterator = runnable.iterator();
            while (iterator.hasNext()) {
                Map.Entry<String, TalosTask> entry = iterator.next();
                TalosTask task = entry.getValue();
    ...
    public String addTask(String name, TalosTask task) {
        Objects.requireNonNull(task, "task");
        for (Map.Entry<String, TalosTask> running : tasks.entrySet()) {
    ...
    public String addNonConflictingTask(String name, TalosTask task) {
        try {
    ...
    public String forceAddTask(String name, TalosTask task) {
    ...
    public int cancel(String nameSubstring) {
        ...
        int cancelled = 0;
        for (Map.Entry<String, TalosTask> entry : tasks.entrySet()) {
    ...
    public int cancelAll() {
        ...
        tasks.values().forEach(TalosTask::_break);
        return cancelled;
    }

    public void onLevelUnload() {
        Iterator<Map.Entry<String, TalosTask>> iterator = tasks.entrySet().iterator();
        while (iterator.hasNext()) {
            TalosTask task = iterator.next().getValue();
    ...
    public Map<String, TalosTask> snapshot() {
        return Collections.unmodifiableMap(new LinkedHashMap<>(tasks));
    }

    private String putWithAutoName(String name, TalosTask task) {
