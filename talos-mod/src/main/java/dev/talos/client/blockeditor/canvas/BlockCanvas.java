package dev.talos.client.blockeditor.canvas;

import dev.talos.client.blockeditor.model.BlockDef;
import dev.talos.client.blockeditor.model.BlockDef.Category;
import dev.talos.client.blockeditor.model.BlockDef.FieldWidget;
import dev.talos.client.blockeditor.model.BlockDef.Shape;
...
import dev.talos.client.blockeditor.registry.BlockRegistry;
...
public final class BlockCanvas {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
        ...
        TalosUi.roundedRect(context, x, y, width, height, 12, palette.rectDark());
        drawGrid(context);
        ...
        TalosUi.roundedRectBorder(context, x, y, width, height, 12, 1, palette.outline());
    ...
    private void drawPalette(DrawContext context, int mouseX, int mouseY) {
        ...
        TalosUi.glassPanel(context, x, y, PALETTE_WIDTH, height, 12, theme.panel());
        ...
        for (int i = 0; i < defs.size(); i++) {
            ...
            TalosUi.roundedRect(context, x + 7, by, PALETTE_WIDTH - 14, 21, def.shape() == Shape.REPORTER ? 10 : 6, color);
    ...
    private void drawTree(DrawContext context, BlockNode node, Set<String> drawn) {
        ...
        TalosUi.glassPanel(context, (float) node.x(), (float) node.y(), w, h,
                node.definition().shape() == Shape.REPORTER ? 14 : 7, color);
        TalosUi.roundedRectBorder(context, (float) node.x(), (float) node.y(), w, h,
    ...
    private void drawChip(DrawContext context, BlockNode node, FieldChip chip) {
        ...
        TalosUi.roundedRect(context, (float) chip.x(), (float) chip.y(), (float) chip.w(), (float) chip.h(), 4,
                editingThis ? 0xFF2C2C2C : 0x99101010);
        TalosUi.roundedRectBorder(context, (float) chip.x(), (float) chip.y(), (float) chip.w(), (float) chip.h(), 4, 1,
    ...
    private void drawBlockPickerChip(DrawContext context, BlockNode node, FieldChip chip, Socket socket) {
        TalosUi.roundedRect(context, (float) chip.x(), (float) chip.y(), (float) chip.w(), (float) chip.h(), 4, 0x99101010);
        TalosUi.roundedRectBorder(context, (float) chip.x(), (float) chip.y(), (float) chip.w(), (float) chip.h(), 4, 1, 0xAAFFFFFF);
    ...
    private void drawPicker(DrawContext context, int mouseX, int mouseY) {
        ...
        TalosUi.glassPanel(context, layout.x, layout.y, layout.w, layout.h, 10, theme.panel());
        TalosUi.roundedRectBorder(context, layout.x, layout.y, layout.w, layout.h, 10, 1, theme.outline());
        var renderer = MinecraftClient.getInstance().textRenderer;
        ...
        for (int row = startRow; row < startRow + visibleRows; row++) {
            for (int col = 0; col < layout.cols; col++) {
                ...
                if (hovered) TalosUi.roundedRect(context, cx, cy, PICKER_CELL - 2, PICKER_CELL - 2, 4, 0x40FFFFFF);
    ...
    private void drawSnapHighlight(DrawContext context, Candidate candidate) {
        ...
        TalosUi.roundedRectBorder(context, (float) target.x() - 2, (float) target.y() - 2, w + 4, h + 4,
                radius, 2, 0xFFFFD34D);
    }
