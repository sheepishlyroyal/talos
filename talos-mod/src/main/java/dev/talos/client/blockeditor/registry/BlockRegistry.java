package dev.talos.client.blockeditor.registry;
...
public final class BlockRegistry {
    ...
    static {
        ...
        add(stack("goto", "go to", List.of(value("x", ValueType.NUMBER, "0"), value("y", ValueType.NUMBER, "64"),
                value("z", ValueType.NUMBER, "0")), "talos.goto({x}, {y}, {z})"));
        add(def("place_block", "place block", Category.ACTION, Shape.STACK,
                ...
                "talos.place_block({x}, {y}, {z}, {block})"));
        add(stack("break_block", "break block", xyz(), "talos.break_block({x}, {y}, {z})"));
        add(stack("kill_nearest", "kill nearest", List.of(), "talos.kill_nearest()"));
        add(stack("wait_between", "wait between", List.of(value("a", ValueType.NUMBER, "0.2"),
                value("b", ValueType.NUMBER, "0.5")), "talos.wait_between({a}, {b})"));
        add(stack("wait", "wait (seconds)", List.of(field("seconds", ValueType.NUMBER, "1.0")),
                "talos.wait_between({seconds}, {seconds})"));
        add(def("find_block", "find block", Category.ACTION, Shape.REPORTER,
                List.of(blockPickerField("name", "minecraft:stone")),
                false, false, ValueType.POSITION, "talos.find_block({name})"));
        add(stack("log", "log", List.of(value("msg", ValueType.ANY, "hello")), "talos.log({msg})"));
        ...
        add(def("player_pos", "player position", Category.VALUE, Shape.REPORTER, List.of(),
                false, false, ValueType.POSITION, "talos.player_pos()"));
