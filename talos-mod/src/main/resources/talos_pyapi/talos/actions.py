...
def place_block(x=None, y=None, z=None, block_id=None):
    ...
    if x is None and y is None and z is None:
        return _talos_host.placeLook()
    if x is None or y is None or z is None:
        raise ValueError("place_block needs all of x, y, z (or none, to place where looking)")
    if block_id is None:
        return _talos_host.placeBlock(int(x), int(y), int(z))
    return _talos_host.placeBlockAs(int(x), int(y), int(z), str(block_id))
