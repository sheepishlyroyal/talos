"""
Type stubs for the `glade` scripting API exposed inside the Minecraft
...
carry no implementation. The real `glade` module is injected into the
...
1. **Drop this file next to your scripts.** Pylance/Pyright will pick up a
   sibling `glade.pyi` automatically for `import glade` completions in that
   folder.
2. **Point `python.analysis.stubPath` at the `stubs/` folder** that ships
   ...
   ```json
   {
     "python.analysis.stubPath": "${workspaceFolder}/.vscode/glade-stubs"
   }
   ```
...
def log(msg: str) -> None:
    """
    Emit a line of text to the Glade output channel in VS Code (streamed
...
def on(event: str) -> Callable[[_EventHandler], _EventHandler]:
    ...
    Example:
        ...
        def handle_damage(source):
            glade.log(f"Took damage from {source}")
    """
    ...
